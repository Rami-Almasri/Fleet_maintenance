...
This document explains *what the system is, what problem it solves, how it is built, and where every part of it lives*. It is meant to be read top to bottom once. After that, §14 tells you which of the other documents in `../` to read for whatever you are actually working on.
...
├── backend/                      ← Laravel 12
│   ├── app/
│   │   ├── Console/Commands/     ← 96 artisan commands (imports, syncs, audits, backfills)
...
│   ├── routes/api.php            ← all 493 routes, one flat grouped file
│   └── tests/
...
├── docs/                         ← design/architecture/audit documents + this handbook
...
**Rules:**
- OM is the **sole source for contracts**. It is **no longer** the source of which cars exist — since **2026-08-19** the "Faster" sheet tab is the fleet register (see below). `importFleetVehicles()` refreshes cars and can never create one; an OM car the register omits is reported as `unlisted` and skipped.
...
`GoogleSheetsService.php` is **read–write** (most importers are read-only, but the vehicle log export writes back). Importers include `VehicleImporter`, `MaintenanceSheetImporter`, `GarageLocationSheetImporter`, and the vehicle-log exporter.

**The fleet register.** The "Faster" tab (`GOOGLE_SHEETS_CARS_ID`, gid `723314813`, header row 3, matched by the **Chassis** column) decides **which cars exist**. `VehicleImporter` creates any car it lists that we do not hold (`origin='sheet'`) and enriches make/model/colour + purchase price; `sync:vehicles` runs it. It must run **BEFORE** the API phase — 02:50, ahead of `om:sync` at 03:00, and first in `fleet:refresh` — because the API can only refresh a car the register already put there. OM keeps owning the operational fields on those cars (plate, year, status, odometer, `car_serial`).

`fleet:register-audit` is the read-only reconciliation: cars we hold that the register omits (mostly sold history — nothing is ever deleted, that would take the car's contracts and repair history with it) and register rows with no car yet.
...
This is the part of the system with the strongest opinions. Read `../Cost-Source-of-Truth.md` and `../Financial-Source-of-Truth.md` before touching money.
...
- **Schema-drift checks miss column types.** A check can report "matches exactly" while a column is `decimal(12,2)` on one side and `decimal(14,2)` on the other. For money, verify types by hand. See `../Invoice-Precision-Drift.md`.
...
`../` holds 83 documents. They vary from current specs to historical design records. **Class docblocks in the code are often more current.** Suggested entry points by purpose:
...
- `../Maintenance-Workflow-Audit.md` · `../Maintenance-Workflow-UseCases.md` · `../Maintenance-Deletion-Model.md`

**Money**
- `../Cost-Source-of-Truth.md` · `../Financial-Source-of-Truth.md` · `../maintenance-financial-architecture.md` · `../Financial-Workflow-Architecture.md` · `../Vehicle-Expense-Provider.md` · `../Invoice-Precision-Drift.md`
- Note `../Invoice-As-Financial-Source-of-Truth.md` describes a **design**, not fully-built code.

**Schema & API**
- `../Backend-Review-Schema-API-Architecture.md`

**Odoo integration specifically**
- [`16-Odoo-Integration.md`](16-Odoo-Integration.md) — the companion to this document: the exact export payload, the expense-provider seam, the Odoo connection facts and the open access blocker.
...
- `../Demo-Live-Walkthrough-AR.md` — an Arabic walkthrough of the live system.

**Intelligence layer** — read the evaluation *before* the architecture, so you know what is real:
- `../Automotive-Knowledge-Platform-Evaluation.md` · `../Intelligence-Platform-As-Built.md` · `../Repair-Intelligence-Architecture.md` · `../Fleet-Knowledge-Engine-Architecture.md`

**Roadmaps and plans** — aspirational; check against code before relying on them:
- `../Implementation-Backlog.md` (the canonical plan) · `../Fleet-Intelligence-Execution-Plan.md` · `../Phase1-Implementation-Plan.md` · `../data-pipeline-roadmap.md`
...
- **The "Faster" sheet says which cars exist. OfficeManager owns contracts. Odoo owns accounting. FleetView owns everything physical and operational in between.**
