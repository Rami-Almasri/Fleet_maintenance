...
### 2a. In the repo — `docs/` (82 markdown files)
...
- **The invoice is the designed financial source of truth**, but that design is only partly implemented — see `docs/Invoice-As-Financial-Source-of-Truth.md` versus `docs/Financial-Source-of-Truth.md`. Do not assume the design doc describes running code.
...
- **The odometer has ~7 writers and no clear owner.** Highest reading wins. If you touch mileage-derived cost (cost/km), read `docs/data-pipeline-roadmap.md` first.
- **Schema-drift checks miss column types.** A drift check can report "matches exactly" while a column is `decimal(12,2)` on one side and `decimal(14,2)` on the other. Verify types by hand when precision matters — which, for money, is always. See `docs/Invoice-Precision-Drift.md`.
...
Of the 82 documents in `docs/`, these are the ones that pay for themselves for an Odoo integrator, roughly in order:

**Essential**
1. `docs/Vehicle-Expense-Provider.md` — the seam you are implementing (short).
2. `docs/maintenance-financial-architecture.md` — where the outbound bridge sits in the money model.
3. `docs/Cost-Source-of-Truth.md` — which number is authoritative and why.
4. `docs/Financial-Source-of-Truth.md` — the as-built financial picture.
5. `ARCHITECTURE-CONVENTIONS.md` (repo root) — before writing PHP here.

**Useful context**
6. `docs/Invoice-As-Financial-Source-of-Truth.md` — the *design* (note: not fully built).
7. `docs/Financial-Workflow-Architecture.md`, `docs/Financial-Layer-Redesign.md`.
8. `docs/Backend-Review-Schema-API-Architecture.md` — schema and API overview.
9. `docs/Parts-Purchase-Repair-Intelligence-Design.md` — where part identity and spend meet, relevant to product mapping.
10. `docs/Invoice-Precision-Drift.md` — decimal precision problems, directly relevant to money mapping.
11. `DEPLOYMENT.md` (repo root).

**Safe to skip.** The bulk of `docs/` covers maintenance intelligence, knowledge engines, recurrence metrics, the asset layer, and demo scripts. Well-written and irrelevant to accounting integration. Skip anything named *Knowledge*, *Intelligence*, *Recurrence*, *Concept-Bridge*, *Asset-Layer*, or *Demo* unless something points you there.
