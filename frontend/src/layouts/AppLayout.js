...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      // Warranty sits BESIDE Parts, not inside it, because it answers the opposite question. The
      // Warranties tab on /parts is a REGISTER — what we were promised. This is the WORK: deciding
      // whether a promise applies before money moves, then chasing the dealer until it does or
      // doesn't. Different people (the warranty desk, not the buyer), different permission
      // (`warranty.view`), and it is where a purchase gets stopped — so burying it as a fifth tab
      // under Parts would have hidden the one surface the whole feature exists to put in front of
      // somebody.
      { name: 'Warranty', to: '/warranty', permission: 'warranty.view', icon: 'M12 3l7 3v6c0 4.4-2.9 7.9-7 9-4.1-1.1-7-4.6-7-9V6zM9.5 12.2l1.8 1.8 3.4-3.6', desc: 'Before we spend money on a car, could the manufacturer, the dealer or a supplier be paying for it instead? The tiles say how much of the fleet is still covered, how much cover is about to run out (on months OR kilometres, whichever comes first) and what has been recovered or avoided. The queue below is the work: coverage reviews somebody must decide before a purchase is allowed, cases waiting on a dealer who has gone quiet, and claims to submit. A purchase request raised on a car with live cover is stopped here, not silently paid for — and every override is recorded with a name and a reason.' },
