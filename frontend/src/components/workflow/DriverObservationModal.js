...
export default function DriverObservationModal({ vehicles = [], onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onDone?.(requestInspection ? 'Observation logged — inspection requested' : 'Observation logged');
    } catch (e) {
      setError(e?.response?.data?.message || 'Could not record the observation');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="md"
      title="Driver observation"
      subtitle="Something you noticed when the car came back — an internal note, not a customer complaint."
      footer={(
        <>
          <Button variant="ghost" onClick={onClose} disabled={saving}>Cancel</Button>
          <Button onClick={submit} disabled={disabled} loading={saving}>
            <Icon.Plus className="h-4 w-4" /> Record
          </Button>
        </>
      )}
    >
      <div className="space-y-4">
        <div>
          <span className="mb-1 block text-sm font-medium text-slate-700">
            Vehicle<span className="ms-0.5 text-red-500">*</span>
          </span>
          <VehicleStatusSelect value={vehicleId} onChange={setVehicleId} vehicles={vehicles} placeholder="Search vehicle…" />
        </div>
        ...
        <Textarea
          ...
          placeholder="e.g. Customer mentioned the steering vibrates · I heard an engine noise bringing it in"
          maxLength={2000}
        />
        ...
        <label className="flex items-start gap-2 rounded-lg bg-slate-50 px-3 py-2.5 text-sm text-slate-600 ring-1 ring-inset ring-slate-100">
          ...
          <span>Raise an inspection request now — sends it to the inspection-review queue. Leave off to just record the note.</span>
        </label>
