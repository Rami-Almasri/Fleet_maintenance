    ...
    <Card className={`p-6 ring-1 ${data?.blocked ? 'ring-red-200' : 'ring-slate-200/60'}`}>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Readiness · 9-point condition check</h3>
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-6 text-slate-400"><Spinner className="h-5 w-5" /><span className="text-sm">Evaluating readiness…</span></div>
      ) : error ? (
        <p className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 ring-1 ring-inset ring-red-600/20">Couldn’t load readiness: {error}</p>
      ) : (
        <>
          {data.blocked && (
            <p className="mb-4 rounded-lg bg-red-50 px-4 py-2.5 text-sm text-red-700 ring-1 ring-inset ring-red-600/20">
              {data.blockers.length} blocking issue{data.blockers.length === 1 ? '' : 's'} — this car is not fit to hand over until resolved.
            </p>
          )}
          ...
          <p className="mt-4 text-xs text-slate-400">
            <span className="font-semibold text-slate-500">Data origin:</span> derived live from condition grade, open workshop tickets,
            flagged damage, registration/insurance expiry, check-in state, cleaning, service interval and GPS —
            the same live authority the rest of the fleet trusts. Nothing here is editable.
          </p>
        </>
      )}
    </Card>
