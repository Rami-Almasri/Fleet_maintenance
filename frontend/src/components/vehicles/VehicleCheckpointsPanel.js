...
function MonitorBanner({ monitor }) {
  ...
  const label = !expected_on ? 'In workshop'
  ...
  return (
    <div className={`flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg px-3 py-2 text-sm ring-1 ${tone}`}>
      ...
      {expected_on && <span className="text-xs opacity-80">Expected {fmtDate(expected_on)}{is_estimated ? ' (estimated)' : ''}</span>}
...
export default function VehicleCheckpointsPanel({ vehicleId }) {
  ...
  return (
    <SectionCard
      ...
      subtitle="Workshop checkpoints — dated progress updates with evidence"
      actions={activeTicketId
        ? <Button size="sm" onClick={() => setOpen(true)}>Submit checkpoint</Button>
        : null}
    >
      {loading ? (
        ...
        <div className="space-y-4">
          {activeTicketId
            ...
            : <p className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-500 ring-1 ring-slate-200">This car is not currently in the workshop.</p>}
