...
// The fallback card lives in its own function component so it can use the i18n
// hook — a class component can't. I18nProvider wraps the whole app (App.js), so
// it is always above this boundary.
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-200/60 bg-white p-8 text-center shadow-soft">
        ...
        <h2 className="text-lg font-bold text-slate-900">{t('errorBoundary.title')}</h2>
        <p className="mt-1 text-sm text-slate-500">{t('errorBoundary.message')}</p>
        {/* The raw error text stays untranslated and LTR — it's developer output. */}
        <p dir="ltr" className="mt-3 break-words rounded-lg bg-slate-50 px-3 py-2 text-start font-mono text-xs text-slate-400">
