  ...
  return (
    <Card className="flex flex-col p-5">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">{phase === 'before' ? t('Step 1') : t('Step 2')}</p>
          <h2 className="text-lg font-bold text-slate-900">{title}</h2>
        </div>
        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-500">{photos.length === 1 ? t('1 photo') : t('{n} photos', { n: photos.length })}</span>
      </div>
      ...
      >
        ...
        {busy ? t('Uploading…') : (phase === 'before' ? t('Take the Before photo') : t('Take the After photo'))}
      </button>
      <input
        ref={inputRef}
        type="file"
        ...
        className="hidden"
      />
      <p className="mt-1.5 text-center text-[11px] text-slate-400">{t('Opens the camera on mobile · compressed in-browser before upload')}</p>
      ...
      <div className="mt-4 flex-1">
        {photos.length === 0 ? (
          <div className="flex h-32 items-center justify-center rounded-xl border border-dashed border-slate-200 text-sm text-slate-400">
            {phase === 'before' ? t('No before photos yet') : t('No after photos yet')}
          </div>
        ) : (
            ...
            {photos.map((p) => (
              <div key={p.id} className="group relative overflow-hidden rounded-lg ring-1 ring-slate-200">
                {p.url ? (
                  ...
                  <div className="flex aspect-square w-full items-center justify-center bg-slate-100 text-[10px] text-slate-400">{t('no preview')}</div>
                )}
                <button
                  type="button"
...
export default function CleaningCapture() {
  ...
  const labelHint = searchParams.get('vehicle') || plateHint || t('Vehicle');
  ...
  const load = useCallback(async () => {
    if (!vehicleId) { setLoading(false); setError(t('No vehicle selected. Open this page from a booking’s Cleaning “Fix” link.')); return; }
    setLoading(true);
    try {
      ...
      setError(e.response?.data?.message || t('Could not load the cleaning record.'));
    } finally {
      setLoading(false);
    }
  }, [vehicleId, t]);
  ...
  const capture = async (phase, file) => {
    ...
    try {
      ...
      setData(d);
      toast.success(phase === 'before' ? t('Before photo saved') : t('After photo saved'));
    } catch (e) {
  ...
  const remove = async (photoId) => {
    try {
      ...
      toast.error(e.response?.data?.message || t('Could not delete the photo'));
    }
  };
  ...
  const mark = async (status) => {
    ...
    try {
      ...
      setData(d);
      toast.success(status === 'clean' ? t('Marked clean — the booking’s Cleaning check is now cleared') : t('Marked dirty'));
    } catch (e) {
  ...
  return (
    <div className="py-8">
      <div className="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
        <PageHeader
          title={t('Vehicle Cleaning')}
          subtitle={t('Capture the car before and after cleaning, then mark it clean to clear the pickup readiness check.')}
        >
          <Link to="/booking-readiness" className="inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 hover:text-indigo-700">
            <span className="inline-block rtl:-scale-x-100">←</span> {t('Back to Booking Readiness')}
          </Link>
        </PageHeader>
        ...
        <Card className="p-4">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            <div className="min-w-0">
              <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">{t('Vehicle')}</p>
            ...
            <div>
              <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">{t('Cleaning status')}</p>
              <Badge tone={statusMeta.tone}>{t(statusMeta.label)}</Badge>
            </div>
            {canManage && (
              <div className="ms-auto flex flex-wrap items-center gap-2">
                ...
                <Button onClick={() => mark('clean')} loading={savingStatus} disabled={!vehicleId} className="gap-1.5">
                  <Icon.Check className="h-4 w-4" /> {t('Mark clean')}
                </Button>
              </div>
            )}
          </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <CaptureColumn
                phase="before" title={t('Before')} hint={t('Shoot the car as it arrived — dirty, so the after is undeniable.')}
                photos={before} busy={busy.before} onCapture={(f) => capture('before', f)} onDelete={remove}
              />
              <CaptureColumn
                phase="after" title={t('After')} hint={t('Shoot the same angles once the car is clean and ready for handover.')}
                photos={after} busy={busy.after} onCapture={(f) => capture('after', f)} onDelete={remove}
              />
            </div>

            <Card className="p-5">
              <h2 className="text-sm font-semibold text-slate-900">{t('Before / After comparison')}</h2>
              <p className="mt-0.5 text-xs text-slate-500">{t('Your latest before vs latest after photo — drag the handle to compare.')}</p>
              <div className="mx-auto mt-3 max-w-xl">
                {compare ? (
                  <BeforeAfterSlider beforeSrc={compare.before} afterSrc={compare.after} beforeLabel={t('Before')} afterLabel={t('After')} />
                ) : (
                  <div className="flex aspect-[4/3] w-full items-center justify-center rounded-2xl border border-dashed border-slate-200 text-sm text-slate-400">
                    {t('Capture a Before and an After photo to compare them here')}
                  </div>
                )}
              </div>
            </Card>
