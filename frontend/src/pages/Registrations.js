...
function CoverageCell({ has, date, days }) {
  ...
  return (
    <div>
      ...
      <div className="mt-1 text-xs text-gray-400">{fmtDate(date)}</div>
    </div>
  );
}
...
function SortHeader({ label, active, onClick }) {
  return (
    <th className="px-6 py-3">
      <button onClick={onClick} className={`group inline-flex items-center gap-1 uppercase tracking-wide transition ${active ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`} title="Sort by soonest expiry">
        {label}
...
function StatTile({ label, value, tone, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`relative flex items-center justify-between rounded-2xl border border-gray-100 bg-white px-5 py-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
        active ? 'ring-2 ring-indigo-500' : 'ring-1 ring-gray-900/5'
      }`}
    >
      <div>
    ...
    <div className="py-8">
      <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          {hasFilters && (
            <button onClick={clearAll} className="rounded-lg px-3 py-2 text-sm font-medium text-gray-500 transition hover:bg-gray-100 hover:text-gray-700">
              Clear
            </button>
          )}
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-100 text-sm stagger-rows">
              <thead className="bg-gray-50/60">
                <tr className="text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                  ...
                  <th className="px-6 py-3">VIN / Chassis</th>
              ...
              {loading ? (
                ...
                <tbody className="divide-y divide-gray-50">
                  {paged.map((r) => (
                    <tr key={r.vehicle_id} className="hover:bg-gray-50/60">
                      <td className="px-6 py-3">
                        <div className="font-medium text-gray-900">{r.plate_no || <span className="text-gray-400">no plate</span>}</div>
                        <div className="text-xs text-gray-400">{[r.make, r.model].filter(Boolean).join(' ') || '—'}</div>
                      </td>
                      <td className="px-6 py-3 font-mono text-xs text-gray-500">{r.chasis_no || r.vin || '—'}</td>
                      <td className="px-6 py-3"><CoverageCell has={r.has_registration} date={r.registration_expiry} days={r.registration_days_left} /></td>
