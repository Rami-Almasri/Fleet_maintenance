...
export default function FleetHealth() {
  ...
  const tabs = useMemo(
    () =>
      [
        ...
        { key: 'readiness', label: t('Vehicle Readiness'), icon: <Icon.Shield className="h-4 w-4" />, show: canAny(['insights.view', 'logistics.view', 'maintenance.view', 'maintenance.initiate', 'maintenance.delegate', 'inspections.view']), Component: ReadinessDashboard },
        { key: 'service', label: t('Service Reminders'), icon: <Icon.Wrench className="h-4 w-4" />, show: can('reminders.view'), Component: ServiceReminders },
        { key: 'bookings', label: t('Booked in Shop'), icon: <Icon.Calendar className="h-4 w-4" />, show: can('maintenance.view'), Component: MaintenanceBookings },
        { key: 'registrations', label: t('Registration & Insurance'), icon: <Icon.Invoice className="h-4 w-4" />, show: can('registration.view'), Component: Registrations },
      ].filter((tab) => tab.show),
    [can, canAny, t],
  );
  ...
  const current = tabs.find((tab) => tab.key === searchParams.get('tab')) || tabs[0];
    ...
    <div>
      <div className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
        <PageHeader
          ...
          subtitle={t("Your fleet's neural center — vehicle readiness, upcoming service, and official documents in one place.")}
        />
        <Tabs tabs={tabs} active={current?.key} onChange={setActive} ariaLabel={t('Fleet health sections')} />
      </div>
