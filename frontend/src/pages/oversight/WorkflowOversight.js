  ...
  return (
    <div className="py-8">
      <div className="mx-auto max-w-[1100px] space-y-8 px-4 sm:px-6 lg:px-8">
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight text-slate-900">{t('oversight.hub.title')}</h1>
          <p className="mt-1 text-sm text-slate-500">{t('oversight.hub.subtitle')}</p>
        </div>
          ...
          {CARDS.map((c) => {
            ...
            return (
              <Link
                key={c.key}
                to={c.to}
                className="group flex flex-col rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md"
              >
