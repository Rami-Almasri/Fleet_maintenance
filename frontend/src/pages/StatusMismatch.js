    ...
    <Card className={`ring-1 ${sev.ring}`}>
      <div className={`border-b px-6 py-4 ${sev.head}`}>
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="text-xs font-medium uppercase tracking-wide opacity-70">{t(sev.label)}</span>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-separate border-spacing-0 text-sm">
          <thead>
            <tr>
              <th className="whitespace-nowrap border-b border-slate-200 bg-slate-50/90 px-5 py-3 text-start text-xs font-semibold uppercase tracking-wide text-slate-500">{t('Car')}</th>
              <th className="whitespace-nowrap border-b border-slate-200 bg-slate-50/90 px-5 py-3 text-start text-xs font-semibold uppercase tracking-wide text-slate-500">{t('Vehicle status')}</th>
              <th className="whitespace-nowrap border-b border-slate-200 bg-slate-50/90 px-5 py-3 text-start text-xs font-semibold uppercase tracking-wide text-slate-500">{t('Contract reality')}</th>
              <th className="whitespace-nowrap border-b border-slate-200 bg-slate-50/90 px-5 py-3 text-start text-xs font-semibold uppercase tracking-wide text-slate-500">{t('Customer')}</th>
            ...
            {g.items.map((it, i) => (
              <tr key={i} className="bg-white transition-colors even:bg-slate-50/40 hover:bg-indigo-50/40">
                ...
                <td className="border-b border-slate-100 px-5 py-3.5 text-end">
                  {it.contract_id
                    ? <Link to={`/contracts/${it.contract_id}`} className="text-xs font-medium text-indigo-600 hover:text-indigo-700">{t('Contract')} <span className="inline-block rtl:-scale-x-100">→</span></Link>
                    : (it.vehicle_id ? <Link to={`/vehicles/${it.vehicle_id}`} className="text-xs font-medium text-indigo-600 hover:text-indigo-700">{t('Car')} <span className="inline-block rtl:-scale-x-100">→</span></Link> : <span className="text-slate-300">—</span>)}
                </td>
              </tr>
            ))}
          </tbody>
...
export default function StatusMismatch({ embedded = false }) {
  ...
  usePageStat({
    ...
    label: t('Out, wrong'),
    color: 'red',
    hint: t('{out} of {total} mismatches are cars out on a contract but not flagged busy', { out: outTotal, total: mismatchTotal }),
  });
      ...
      {error && (
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-200/60 bg-white px-5 py-4 shadow-soft">
            <p className="text-xs font-medium text-slate-500">{t('Total mismatches')}</p>
            <p className="mt-1 font-display text-2xl font-bold tracking-tight text-slate-900">{num(data?.total_mismatches)}</p>
          </div>
          <div className="rounded-2xl border border-slate-200/60 bg-white px-5 py-4 shadow-soft">
            <p className="flex items-center gap-1.5 text-xs font-medium text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-red-500" />{t('Out, status wrong')}</p>
            <p className="mt-1 font-display text-2xl font-bold tracking-tight text-red-600">{num(outCases)}</p>
          </div>
          <div className="rounded-2xl border border-slate-200/60 bg-white px-5 py-4 shadow-soft">
            <p className="flex items-center gap-1.5 text-xs font-medium text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-amber-500" />{t('Stale (no contract)')}</p>
            <p className="mt-1 font-display text-2xl font-bold tracking-tight text-amber-600">{num(staleCases)}</p>
          </div>
        </div>

      {groups.length === 0
        ? <Card><EmptyState title={t('No status mismatches 🎉')} message={t("Every car's status matches its open contracts.")} /></Card>
  ...
  return (
    <div className="py-8">
      <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        <PageHeader title={t('Status Mismatches')} subtitle={t("Cars whose status doesn't match their contracts — out on a contract but not flagged busy, or flagged busy with no open contract.")} />
        {inner}
      </div>
    </div>
  );
